...
public class CameraControl : MonoBehaviour {
	...
	public float cameraSizeChangeDelta = 2;  
	...
	void FixedUpdate(){
		...
		if (!bothInCamera && !cameraLock){
			StartCoroutine (changeCameraSize (Camera.main.orthographicSize, maxOrthographicSize, 0.5f, Time.time,
			                                  Camera.main.transform.position, levelBounds.transform.position));
		}else if (bothInCamera && dist <= 5 && Camera.main.orthographicSize != minOrthographicSize && !cameraLock) {
			StartCoroutine (changeCameraSize (Camera.main.orthographicSize, minOrthographicSize, 0.5f, Time.time, 
